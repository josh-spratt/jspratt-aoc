...
def monkey_investigation(monkey, item):
    ...
    while x <= 20:
        for j in range(0, len(monkey_list)):
            while len(monkey_list[j].items) > 0:
                print(monkey_list[j].__dict__)
                print(monkey_list[j].items[0])
                try:
                    evals_to, value = monkey_investigation(monkey_list[j], monkey_list[j].items[0])
